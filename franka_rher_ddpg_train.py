...
from franka_env import ManagerRLGoalEnv, FrankaShadowMultiTaskLiftEnvCfg
...
ENV_CFG = FrankaShadowMultiTaskLiftEnvCfg()
...
class DetPolicyNetwork(nn.Module):
    def __init__(
        ...
        self.net = SeqGRUNet(
            ...
            out_dim=self.n_tasks * self.act_dim,
    ...
    def forward(self, x: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        return self.net(obs, meta, mask).view(B, self.n_tasks, self.act_dim)[batch_idxs, task_idxs] # (mu, std)
...
if __name__ == '__main__':
    ...
    class EnvCfgWithSeed(FrankaShadowMultiTaskLiftEnvCfg):
        def __init__(self):
            super().__init__()
